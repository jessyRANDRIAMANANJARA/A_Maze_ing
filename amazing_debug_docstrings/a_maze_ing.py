"""Entry point: parse the config file, then run the maze application."""
...
def check_42_pattern_avilability(height: int, width: int) -> str:
    """Check whether a maze of the given size can fit the "42" pattern.
    ...
    -------
    str
        An empty string if the pattern fits (`width` >= 14 and
        `height` >= 10), otherwise a message explaining that it was
        omitted.
    """
...
def main() -> None:
    """Read the config file given on the command line and run the app.

    Validates the number of arguments and that the config file exists
    and is readable, parses it via `config_reader.read_config`, then
    builds and runs the interactive `render.Terminal` session with the
    algorithm and settings from the config file.
